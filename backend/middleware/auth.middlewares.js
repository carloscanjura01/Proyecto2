...
middleware.authentication = async (req, res, next) => {
  try {
    ...
    if (prefix !== PREFIX) {
      return res.status(401).json({ error: "User not authenticatedss" });
    }

    if (!token) {
      return res.status(401).json({ error: "User not authenticated ss" });
    }
...
middleware.authorization = (roleRequired = ROLES.SYSADMIN) => {
  return (req, res, next) => {
    // Premisa antes de este middleware debe de haber pasado por la autentificacion
    try {
